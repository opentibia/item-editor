		...
		public bool LoadDat(string filename, UInt32 signature)
		{
			...
			try
			{
				using (BinaryReader reader = new BinaryReader(fileStream))
				{
					...
					{
						string message = "Plugin1030: Bad dat signature. Expected signature is {0:X} and loaded signature is {1:X}.";
					...
					while (id <= maxclientID)
					{
						...
						do
						{
							...
							switch (flag)
							{
								...
								default:
									Trace.WriteLine(String.Format("Plugin1030: Error while parsing, unknown flag 0x{0:X} at id {1}.", flag, id));
									return false;
							}
