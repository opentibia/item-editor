...
namespace TibiaCustom
{
	public class TibiaCustom : SpriteItem
	{
		public bool hasCharges;

		public override bool IsEqual(Item item)
		{
			if (item is TibiaCustom)
			{
				if(((TibiaCustom)item).hasCharges != hasCharges)
				{
					return false;
				}
			}
	...
	public class Plugin : IPlugin
	{
		...
		{
			settings.Load("pluginCustom.xml");
		...
		public bool LoadDat(string filename, UInt32 signature)
		{
			...
			try
			{
				using (BinaryReader reader = new BinaryReader(fileStream))
				{
					...
					{
						string message = "TibiaCustom: Bad dat signature. Expected signature is {0:X} and loaded signature is {1:X}.";
					...
					{
						TibiaCustom item = new TibiaCustom();
						item.id = id;
						items[id] = item;
